...
/**
 * Class WC_Gateway_Bank_BTN file.
 *
...
class WC_Gateway_BTN extends WC_Payment_Gateway
{
    ...
    public function generate_account_details_html()
    {
        ...
        <tr valign="top">
            ...
            <td class="forminp" id="bank_btn_accounts">
        ...
        if ($this->account_details) {
            foreach ($this->account_details as $account) {
                ...
                echo '<tr class="account">
										<td class="sort"></td>
										<td><input type="text" value="' . esc_attr(wp_unslash($account['account_name'])) . '" name="bank_btn_account_name[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['account_number']) . '" name="bank_btn_account_number[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['sort_code']) . '" name="bank_btn_sort_code[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['iban']) . '" name="bank_btn_iban[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['bic']) . '" name="bank_btn_bic[' . esc_attr($i) . ']" /></td>
									</tr>';
            }
        }
        ?>
...
<?php
    ...
    private function bank_details($order_id = '')
    {
        ...
        if (! empty($bank_btn_accounts)) {
            ...
            foreach ($bank_btn_accounts as $bank_btn_account) {
                ...
                if ($bank_btn_account->account_name) {
                    $account_html .= '<h3 class="wc-bank-btn-bank-details-account-name">' . wp_kses_post(wp_unslash(implode(' - ', array_filter([$bank_btn_account->account_name ?? null, $this->method_title ?? null])))) . ':</h3>' . PHP_EOL;
                }

                $account_html .= '<ul class="wc-bank-btn-bank-details order_details bacs_details">' . PHP_EOL;
            ...
            if ($has_details) {
                echo '<section class="woocommerce-bank-btn-bank-details"><h2 class="wc-bank-btn-bank-details-heading">' . esc_html__('Rincian rekening bank kami', 'indobe-for-woocommerce') . '</h2>' . wp_kses_post(PHP_EOL . $account_html) . '</section>';
            }
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if ($order->get_total() > 0) {
            /**
             * Filter the order status for Bank BTN payment.
             *
             * @since 3.4.0
             *
             ...
             * @param object $order          The order object.
             */
            $process_payment_status = apply_filters('indobe_bank_btn_process_payment_order_status', OrderStatus::ON_HOLD, $order);
            // Mark as on-hold (we're awaiting the payment).
            $order->update_status($process_payment_status, __('Menunggu pembayaran dari Bank BTN.', 'indobe-for-woocommerce'));
        } else {
