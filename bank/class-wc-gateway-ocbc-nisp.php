...
class WC_Gateway_OCBC_NISP extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->icon               = apply_filters('indobe_bank_ocbc_nisp_icon', $icon_url);
        $this->has_fields         = false;
        $this->method_title       = __('Bank OCBC NISP', 'indobe-for-woocommerce');
        $this->method_description = __('Lakukan pembayaran melalui transfer langsung ke rekening Bank OCBC NISP.', 'indobe-for-woocommerce');
        ...
        $this->account_details = get_option(
            'indobe_bank_ocbc_nisp_accounts',
            array(
                array(
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            'enabled'         => array(
                ...
                'type'    => 'checkbox',
                'label'   => __('Enable Bank OCBC NISP', 'indobe-for-woocommerce'),
                'default' => 'no',
            ),
            'title'           => array(
                ...
                'default'     => __('Transfer Bank OCBC NISP', 'indobe-for-woocommerce'),
                'desc_tip'    => true,
            ),
            'enable_icon' => array(
            ...
            'description'     => array(
                ...
                'default'     => __('Lakukan pembayaran langsung ke rekening Bank OCBC NISP kami. Mohon gunakan ID Pesanan Anda sebagai referensi pembayaran. Pesanan Anda tidak akan dikirimkan hingga dana telah masuk ke rekening kami.', 'indobe-for-woocommerce'),
                'desc_tip'    => true,
            ),
            'instructions'    => array(
...
<?php
    ...
    public function save_account_details()
    {
        ...
        do_action('woocommerce_update_option', array('id' => 'indobe_bank_ocbc_nisp_accounts'));
        update_option('indobe_bank_ocbc_nisp_accounts', $accounts);
    }
    ...
    public function email_instructions($order, $sent_to_admin, $plain_text = false)
    {
        if (! $sent_to_admin && self::ID === $order->get_payment_method()) {
             ...
             */
            $instructions_order_status = apply_filters('indobe_bank_ocbc_nisp_email_instructions_order_status', OrderStatus::ON_HOLD, $order);
    ...
    private function bank_details($order_id = '')
    {
        ...
        $bank_ocbc_nisp_accounts = apply_filters('indobe_bank_ocbc_nisp_accounts', $this->account_details, $order_id);
        ...
        if (! empty($bank_ocbc_nisp_accounts)) {
            ...
            foreach ($bank_ocbc_nisp_accounts as $bank_ocbc_nisp_account) {
                ...
                $account_fields = apply_filters(
                    'indobe_bank_ocbc_nisp_account_fields',
                    array(
            ...
            if ($has_details) {
                echo '<section class="woocommerce-bank-ocbc-nisp-bank-details"><h2 class="wc-bank-ocbc-nisp-bank-details-heading">' . esc_html__('Rincian rekening bank kami', 'indobe-for-woocommerce') . '</h2>' . wp_kses_post(PHP_EOL . $account_html) . '</section>';
            }
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if ($order->get_total() > 0) {
             ...
             * @param object $order          The order object.
             */
            $process_payment_status = apply_filters('indobe_bank_ocbc_nisp_process_payment_order_status', OrderStatus::ON_HOLD, $order);
            // Mark as on-hold (we're awaiting the payment).
            $order->update_status($process_payment_status, __('Menunggu pembayaran dari Bank OCBC NISP.', 'indobe-for-woocommerce'));
        } else {
