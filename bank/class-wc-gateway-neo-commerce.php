...
/**
 * Class WC_Gateway_Bank_Neo_Commerce file.
 *
...
/**
 * Bank Neo Commerce Payment Gateway.
 *
 * Provides a Bank Neo Commerce Payment Gateway. Based on standard WooCommerce BACS.
 *
 * @class       WC_Gateway_Neo Commerce
 * @extends     WC_Payment_Gateway
 * @version     4.0.0
 * @package     WooCommerce\Classes\Payment
 */
class WC_Gateway_Neo_Commerce extends WC_Payment_Gateway
{
    /**
     ...
     * @var string
     */
    public const ID = 'bank_neo_commerce';
        ...
        if ($this->account_details) {
            foreach ($this->account_details as $account) {
                ...
                echo '<tr class="account">
										<td class="sort"></td>
										<td><input type="text" value="' . esc_attr(wp_unslash($account['account_name'])) . '" name="bank_neo_commerce_account_name[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['account_number']) . '" name="bank_neo_commerce_account_number[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['sort_code']) . '" name="bank_neo_commerce_sort_code[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['iban']) . '" name="bank_neo_commerce_iban[' . esc_attr($i) . ']" /></td>
										<td><input type="text" value="' . esc_attr($account['bic']) . '" name="bank_neo_commerce_bic[' . esc_attr($i) . ']" /></td>
									</tr>';
            }
        }
        ?>
...
<?php
    ...
    public function save_account_details()
    {
        ...
        if (
            isset($_POST['bank_neo_commerce_account_name']) && isset($_POST['bank_neo_commerce_account_number'])
            && isset($_POST['bank_neo_commerce_sort_code']) && isset($_POST['bank_neo_commerce_iban']) && isset($_POST['bank_neo_commerce_bic'])
        ) {
            $account_names   = wc_clean(wp_unslash($_POST['bank_neo_commerce_account_name']));
            $account_numbers = wc_clean(wp_unslash($_POST['bank_neo_commerce_account_number']));
            $sort_codes      = wc_clean(wp_unslash($_POST['bank_neo_commerce_sort_code']));
            $ibans           = wc_clean(wp_unslash($_POST['bank_neo_commerce_iban']));
            $bics            = wc_clean(wp_unslash($_POST['bank_neo_commerce_bic']));
    ...
    private function bank_details($order_id = '')
    {
        ...
        if (! empty($bank_neo_commerce_accounts)) {
            ...
            foreach ($bank_neo_commerce_accounts as $bank_neo_commerce_account) {
                $bank_neo_commerce_account = (object) $bank_neo_commerce_account;

                if ($bank_neo_commerce_account->account_name) {
                    $account_html .= '<h3 class="wc-bank-neo-commerce-bank-details-account-name">' . wp_kses_post(wp_unslash(implode(' - ', array_filter([$bank_neo_commerce_account->account_name ?? null, $this->method_title ?? null])))) . ':</h3>' . PHP_EOL;
                }

                $account_html .= '<ul class="wc-bank-neo-commerce-bank-details order_details bacs_details">' . PHP_EOL;
    ...
    public function process_payment($order_id)
    {
        ...
        if ($order->get_total() > 0) {
            /**
             * Filter the order status for Bank Neo Commerce payment.
             *
             * @since 3.4.0
             *
    ...
    public function get_country_locale()
    {
        if (empty($this->locale)) {
            ...
            $this->locale = apply_filters(
                'woocommerce_get_bank_neo_commerce_locale',
                array(
                    'AU' => array(
                        'sortcode' => array(
